...
TOOL_VER = "v21.0 | Stand: 13.05.2026"
...
class SmartCaptureApp:
    ...
    def setup_info_tab(self):
        ...
        tk.Label(pad, text="Version: v21.0 | Stand: 13.05.2026", font=("Segoe UI", 10), bg=BG_COLOR, fg=DARK_COLOR).pack(anchor="w", pady=(0, 15))
    ...
    def show_preview(self, label, img):
        ...
        if getattr(self, "preview_mode", "horizontal") == "vertical":
            ...
            target_w = (self.root.winfo_width() - 150) // 2
            max_h = 200
            
        if target_w < 150: target_w = 200
    ...
    def start_ocr(self):
        ...
        for i, f in enumerate(files):
            lbl.config(text=self.t("progress_step").format(i=i+1, n=len(files)))
            pw.update()
            try:
                img = Image.open(f)
